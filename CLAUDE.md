# Architecture — yral-hello-world-counter

Counter service at `dolr-ai/yral-hello-world-counter`.
Live at `https://hello-world-counter.rishi.yral.com`
...
| `patroni/post_init.sh` | Runs ONCE on first bootstrap: creates counter_db + counter table |
...
| `DATABASE_URL_SERVER_1` | `postgresql://postgres:POSTGRES_PASSWORD@haproxy-rishi-1:5432/counter_db` |
| `DATABASE_URL_SERVER_2` | `postgresql://postgres:POSTGRES_PASSWORD@haproxy-rishi-2:5432/counter_db` |
...
│   └── docker-compose.yml
├── counter-db-stack/           ← Swarm stack files (CI uploads here)
│   ├── etcd-stack.yml
│   ├── patroni-stack.yml
│   ├── haproxy-stack.yml
│   └── haproxy.cfg
...
/home/deploy directories. Check them with: `docker stack services counter-db`
...
Always use `docker stack rm counter-db` (removes stack) or `docker service update` for targeted restarts.

### The Caddyfile must include ALL service blocks
This Caddyfile has both `hello-world` and `hello-world-counter` blocks.
...
`docker service update --force counter-db_etcd-rishi-1`
